...
export const JobCard: React.FC<JobCardProps> = ({ job, userGradYear, onSwipe, onClick, className }) => {
  ...
  return (
    <motion.div
      ...
      className={twMerge(
        'absolute w-full max-w-sm h-[500px] rounded-2xl shadow-xl cursor-grab active:cursor-grabbing flex flex-col justify-between p-6 border border-slate-200',
        className
      )}
      onClick={() => onClick(job)}
    ...
    >
      <div>
        ...
        <p className="text-lg text-slate-500 font-medium mb-4">{job.company}</p>
        ...
        {!isGradMatch && (
           <div className="flex items-center gap-2 text-rose-500 text-sm font-medium bg-rose-50 px-3 py-2 rounded-lg">
