...
export const AiAssistant = forwardRef<AiAssistantRef, AiAssistantProps>(({ userId, jobs = [], applications = [] }, ref) => {
  ...
  const transcribeAudio = async (audioBlob: Blob) => {
    ...
    try {
      ...
      // Try Local Whisper first (port 3001)
      console.log('🎤 Trying Local Whisper...');
      try {
        const localResponse = await fetch('http://localhost:3001/v1/audio/transcriptions', {
          method: 'POST',
          body: formData
        });
        ...
        if (localResponse.ok) {
          ...
          if (data.text) {
            console.log('✅ Local Whisper succeeded');
            setInputValue(prev => prev + (prev ? ' ' : '') + data.text);
            return;
          }
        }
      } catch (localError) {
        console.log('⚠️ Local Whisper unavailable, falling back to Groq...');
      }

      // Fallback to Groq Cloud (port 5000)
      const groqFormData = new FormData();
      groqFormData.append('audio', audioBlob, 'recording.webm');

      const groqResponse = await fetch('http://localhost:5000/api/ai/transcribe', {
        method: 'POST',
        body: groqFormData
      });

      if (!groqResponse.ok) {
        throw new Error('Both local and cloud transcription failed');
      }

      const data = await groqResponse.json();
      if (data.text) {
        console.log('✅ Groq Cloud transcription succeeded');
        setInputValue(prev => prev + (prev ? ' ' : '') + data.text);
      }
      ...
      {!isOpen ? (
        ...
        <div className="w-full h-full bg-white rounded-2xl shadow-2xl flex flex-col border border-slate-200 overflow-hidden relative">
          ...
          <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-slate-100 flex items-end gap-2">
            <textarea value={inputValue} onChange={(e) => { setInputValue(e.target.value); e.target.style.height = 'auto'; e.target.style.height = `${Math.min(e.target.scrollHeight, 150)}px`; }} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(); } }} placeholder={isRecording ? "🎤 Listening..." : isTranscribing ? "✨ Transcribing..." : "Message MoubAssistant..."} rows={1} className={clsx("flex-1 px-4 py-2.5 bg-slate-100 border-none rounded-2xl text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none resize-none overflow-y-auto max-h-[150px]", isRecording && "ring-2 ring-red-400 bg-red-50")} disabled={isTranscribing} />
            <button
              type="button"
              ...
              className={clsx(
                "p-2.5 rounded-xl transition-all duration-200 relative border",
                isRecording
                  ? "bg-red-500 text-white border-red-500 animate-pulse"
                  : "bg-white border-slate-200 text-slate-400 hover:text-indigo-600 hover:border-indigo-200 hover:bg-slate-50",
