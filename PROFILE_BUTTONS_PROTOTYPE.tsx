...
import { JobDeck } from './src/components/JobDeck';
import { Tracker } from './src/components/Tracker';
import { JobDetailModal } from './src/components/JobDetailModal';
import { ReviewModal } from './src/components/ReviewModal';
import { AiAssistant } from './src/components/AiAssistant';
import { type Job } from './src/components/JobCard';
...
function AppContent() {
    ...
    const isProfileComplete = () => {
        ...
        return !!(
            pd.phone && pd.address && pd.city && pd.state && pd.zip &&
            pd.linkedin && pd.university && pd.degree &&
            demo.gender && demo.race && demo.veteran && demo.disability &&
            common.workAuth && common.sponsorship && common.relocation && common.formerEmployee &&
            essay.whyExcited && essay.howDidYouHear
        );
    };
    ...
    useEffect(() => {
        if (user) {
            setProfileFormData({
                personalDetails: { ...profileFormData.personalDetails, ...(user.personalDetails || {}) },
                demographics: { ...profileFormData.demographics, ...(user.demographics || {}) },
                commonReplies: { ...profileFormData.commonReplies, ...(user.commonReplies || {}) },
                customAnswers: { ...profileFormData.customAnswers, ...(user.customAnswers || {}) },
                essayAnswers: { ...profileFormData.essayAnswers, ...(user.essayAnswers || {}) }
            });
        }
    }, [user]);
    ...
    const updateProfileField = (section: keyof typeof profileFormData, field: string, value: string) => {
                            ...
                            <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 transition-colors">
                                ...
                                <div className="mt-8 pt-8 border-t border-slate-200 dark:border-slate-800">
                                    ...
                                    <div className="grid grid-cols-1 gap-6">
                                        <label className="block">
                                            <span className="text-xs font-bold text-slate-500 uppercase">Why do you want to join? (Generic "Why Us")</span>
